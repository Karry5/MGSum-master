...
class HierarchicalTransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, block_mask, doc_lengths, doc_block_mask):
        ...
        doc_padding_mask = ~ doc_padding_mask.lt(doc_lengths.unsqueeze(1).cpu())
        ...
        block_padding_mask = torch.sum(~local_padding_mask.view(batch_size, n_blocks, n_tokens), -1) == 0
        ...
        src_features = src_features.reshape(batch_size, n_blocks * n_tokens, -1)
...
class TransformerLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, block_vec, doc_vec, local_padding_mask, block_padding_mask, doc_padding_mask, block_mask, doc_block_mask, batch_size, n_blocks):
        ...
        inter_padding_mask = torch.cat([torch.zeros(local_padding_mask.size(0), 1).cuda().byte(), local_padding_mask.byte()], -1)
...
class HierarchicalTransformer_with_copyDecoder(transformer_with_copyDecoder):
    ...
    def get_normalized_probs(self, net_output, log_probs, sample):
        ...
        mask_hier = ~local_padding_mask[:, :, None]
